...
class Network:
    ...
    def _init_graph(self) -> None:
        ...
        self.input_shape = self.input_shapes[1]
    ...
    def copy_own_vars_from(self, src_net: "Network") -> None:
        names = [name for name in self.own_vars.keys() if "G_synthesizer/" + name in src_net.own_vars]
        tfutil.set_vars(tfutil.run({self.vars[name]: src_net.vars["G_synthesizer/" + name] for name in names}))

    def copy_vars_from(self, src_net: "Network") -> None:
         
        names = [name for name in self.vars.keys() if "G_synthesizer/" + self.translate(name) in src_net.vars]
        uninitialized = [name for name in self.vars.keys() if "G_synthesizer/" + self.translate(name) not in src_net.vars]
    ...
    def copy_trainables_from(self, src_net: "Network") -> None:
        ...
        names = [name for name in self.trainables.keys() if "G_synthesizer/" + name in src_net.trainables]
        tfutil.set_vars(tfutil.run({self.vars[name]: src_net.vars["G_synthesizer/" + name] for name in names}))
